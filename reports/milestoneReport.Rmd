...
The question of how many unique words would cover most of our word instances.
...
The graph of the **number of terms** versus **cumulative proportion** reveals that only `r dim(filter(c_ng1, Cumulative <= .500))[1]` terms are needed to cover 50% of words instances and `r dim(filter(c_ng1, Cumulative <= .900))[1]` for 90%, in a +n universe of `r dim(c_ng1)[1]` terms.
...
Taking in account the prediction model, it can que usefull for reducing size and runtime to work only with the 90% coverage ngrams.
...
langPlot <- langPlot + facet_wrap( ~ ngram, nrow = 3) + theme(legend.position = "none")
...
The package `RDRPOSTagger` can be helpfull in this subject.
